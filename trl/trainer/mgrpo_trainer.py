...
class MGRPOTrainer(Trainer):
    ...
    Args:
        ...
        args ([`MGRPOConfig`], *optional*, defaults to `None`):
    ...
    def __init__(
        ...
        args: Optional[MGRPOConfig] = None,
        ...
        if args is None:
            ...
            args = MGRPOConfig(f"{model_name}-GRPO")
        ...
        if isinstance(model, str):
            ...
            if isinstance(torch_dtype, torch.dtype) or torch_dtype == "auto" or torch_dtype is None:
                ...
                raise ValueError(
                    "Invalid `torch_dtype` passed to `MGRPOConfig`. Expected either 'auto' or a string representing "
            ...
            if args.model_init_kwargs is not None:
                raise ValueError(
                    "You passed `model_init_kwargs` to the `MGRPOConfig`, but your model is already instantiated. "
        ...
        self.max_draft_length = args.max_draft_length 
        ...
        if (
            ...
            raise NotImplementedError(
                "Iterable datasets are not yet supported in MGRPOTrainer. Please use a standard dataset instead."
            )
        ...
        if self.use_vllm:
            ...
            self.draft_config = GenerationConfig(
                max_new_tokens=self.max_draft_length,
                do_sample=True,
                pad_token_id=processing_class.pad_token_id,
                bos_token_id=processing_class.bos_token_id,
                eos_token_id=processing_class.eos_token_id,
    ...
    def _set_signature_columns_if_needed(self):
        ...
        # In MGRPOTrainer, we preprocess data, so using the model's signature columns doesn't work.
    ...
    def _enable_gradient_checkpointing(self, model: PreTrainedModel, args: MGRPOConfig) -> PreTrainedModel:
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        if return_outputs:
            raise ValueError("The MGRPOTrainer does not support returning outputs")
        if self.use_liger_loss:
    ...
    def _compute_loss(self, model, inputs):
        ...
        draft_coef_1 = torch.exp(draft_per_token_logps - old_draft_per_token_logps)
        draft_coef_2 = torch.clamp(draft_coef_1, 1 - self.epsilon_low, 1 + self.epsilon_high)
        draft_per_token_loss1 = draft_coef_1 * draft_advantages.unsqueeze(1)
        draft_per_token_loss2 = draft_coef_2 * draft_advantages.unsqueeze(1)
        draft_per_token_loss = -torch.min(draft_per_token_loss1, draft_per_token_loss2)

        solution_coef_1 = torch.exp(solution_per_token_logps - old_solution_per_token_logps)
        solution_coef_2 = torch.clamp(solution_coef_1, 1 - self.epsilon_low, 1 + self.epsilon_high)
        solution_per_token_loss1 = solution_coef_1 * solution_advantages.unsqueeze(1)
