...
class DFPOTrainer(Trainer):
    ...
    Args:
        ...
        args ([`DFPOConfig`], *optional*, defaults to `None`):
    ...
    def __init__(
        ...
        if args is None:
            ...
            args = DFPOConfig(f"{model_name}-GRPO")
        ...
        if isinstance(model, str):
            ...
            if isinstance(torch_dtype, torch.dtype) or torch_dtype == "auto" or torch_dtype is None:
                ...
                raise ValueError(
                    "Invalid `torch_dtype` passed to `DFPOConfig`. Expected either 'auto' or a string representing "
            ...
            if args.model_init_kwargs is not None:
                raise ValueError(
                    "You passed `model_init_kwargs` to the `DFPOConfig`, but your model is already instantiated. "
        ...
        if (
            ...
            raise NotImplementedError(
                "Iterable datasets are not yet supported in DFPOTrainer. Please use a standard dataset instead."
            )
    ...
    def _set_signature_columns_if_needed(self):
        ...
        # In DFPOTrainer, we preprocess data, so using the model's signature columns doesn't work.
    ...
    def _enable_gradient_checkpointing(self, model: PreTrainedModel, args: DFPOConfig) -> PreTrainedModel:
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        if return_outputs:
            raise ValueError("The DFPOTrainer does not support returning outputs")
        if self.use_liger_loss:
